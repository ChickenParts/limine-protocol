...
Values assignable to `mode`, `max_mode`, and `min_mode`:
```c
#define LIMINE_PAGING_MODE_MIPS64_4LVL 0

#define LIMINE_PAGING_MODE_DEFAULT LIMINE_PAGING_MODE_MIPS64_4LVL
...
Values assignable to `mode`, `max_mode`, and `min_mode`:
```c
#define LIMINE_PAGING_MODE_IA64_4LVL 0

#define LIMINE_PAGING_MODE_DEFAULT LIMINE_PAGING_MODE_IA64_4LVL
...
* `flags` - Always zero
* `bsp_processor_id` - Processor ID of the bootstrap processor.
* `cpu_count` - How many CPUs are present. It includes the bootstrap processor.
* `cpus` - Pointer to an array of `cpu_count` pointers to
`struct limine_mp_info` structures.
...
* `flags` - Always zero
* `bsp_tpc` - TPC of the bootstrap processor.
* `cpu_count` - How many CPUs are present. It includes the bootstrap processor.
* `cpus` - Pointer to an array of `cpu_count` pointers to
`struct limine_mp_info` structures.
...
* `tpc` - The TPC of the CPU.
* `goto_address` - An atomic write to this field causes the parked CPU to
jump to the written address, on a 64KiB (or Stack Size Request size) stack. A pointer to the
`struct limine_mp_info` structure of the CPU is passed in `o0`. Other than
that, the CPU state will be the same as described for the bootstrap
processor. This field is unused for the structure describing the bootstrap
processor.
* `extra_argument` - A free for use field.
...
* `flags` - Always zero
* `bsp_cpu_address` - CPU address of the bootstrap processor.
* `cpu_count` - How many CPUs are present. It includes the bootstrap processor.
* `cpus` - Pointer to an array of `cpu_count` pointers to
`struct limine_mp_info` structures.
...
* `cpu_address` - The CPU address of the CPU.
* `goto_address` - An atomic write to this field causes the parked CPU to
jump to the written address, on a 64KiB (or Stack Size Request size) stack. A pointer to the
`struct limine_mp_info` structure of the CPU is passed in `r2`. Other than
that, the CPU state will be the same as described for the bootstrap
processor. This field is unused for the structure describing the bootstrap
processor.
* `extra_argument` - A free for use field.
...
* `flags` - Always zero
* `bsp_lid` - LID of the bootstrap processor.
* `cpu_count` - How many CPUs are present. It includes the bootstrap processor.
* `cpus` - Pointer to an array of `cpu_count` pointers to
`struct limine_mp_info` structures.
...
* `flags` - Always zero
* `bsp_cpuid` - CPU ID of the bootstrap processor.
* `cpu_count` - How many CPUs are present. It includes the bootstrap processor.
* `cpus` - Pointer to an array of `cpu_count` pointers to
`struct limine_mp_info` structures.
...
* `cpuid` - The CPU ID of the CPU.
* `goto_address` - An atomic write to this field causes the parked CPU to
jump to the written address, on a 64KiB (or Stack Size Request size) stack. A pointer to the
`struct limine_mp_info` structure of the CPU is passed in `rp2`. Other than
that, the CPU state will be the same as described for the bootstrap
processor. This field is unused for the structure describing the bootstrap
processor.
* `extra_argument` - A free for use field.
...
struct limine_mips64_cpu_features_request {
    uint64_t id[4];
    uint64_t revision;
    struct limine_mips64_cpu_features_response *response;
};
```
...
### SPARC V9 CPU Features Feature

ID:
```c
#define LIMINE_SPARCV9_CPU_FEATURES_REQUEST { LIMINE_COMMON_MAGIC, 0xa9b8c7d6e5f43210, 0x1234567890abcdef }
```

Request:
```c
struct limine_sparcv9_cpu_features_request {
    uint64_t id[4];
    uint64_t revision;
    struct limine_sparcv9_cpu_features_response *response;
};
```

Response:
```c
struct limine_sparcv9_cpu_features_response {
    uint64_t revision;
    uint64_t ver;
};
```

* `ver` - The VER (Version) register of the CPU.

### z/Architecture CPU Features Feature

ID:
```c
#define LIMINE_ZARCH_CPU_FEATURES_REQUEST { LIMINE_COMMON_MAGIC, 0xb8c7d6e5f43210a9, 0x234567890abcdef1 }
```

Request:
```c
struct limine_zarch_cpu_features_request {
    uint64_t id[4];
    uint64_t revision;
    struct limine_zarch_cpu_features_response *response;
};
```

Response:
```c
struct limine_zarch_cpu_features_response {
    uint64_t revision;
    uint64_t facilities[8];
};
```

* `facilities` - The facility bits of the CPU.

### Itanium CPU Features Feature
...
struct limine_ia64_cpu_features_request {
    uint64_t id[4];
    uint64_t revision;
    struct limine_ia64_cpu_features_response *response;
};
```
...
### PA-RISC 2.0 CPU Features Feature
...
struct limine_parisc_cpu_features_request {
    uint64_t id[4];
    uint64_t revision;
    struct limine_parisc_cpu_features_response *response;
};
```
